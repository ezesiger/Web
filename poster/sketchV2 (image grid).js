...
function setup() {
  ...
  gridIframe = document.querySelector('iframe');
}
...
function draw() {
  ...
  if (cycleSpeed <= 116 && cycleSpeed >= 81) {
    ...
    if (points.length === 0 || currentHorseImage !== moto) {
      ...
      currentHorseImage = moto;
    }
  } else {
    // normal horse cycling behavior
...
function drawTitle() {
  ...
  //title timings based on cycle speed
...
function drawCaption() {
  ...
  //caption timings based on cycle speed
...
function generateHorsePoints(horseIndex){
  ...
  // align image
  let offsetX = (width - img.width + bumpX);
  let offsetY = (height - img.height + bumpY);
...
function generateMotoPoints() {
  ...
  // align image
  let offsetX = (width - displayImg.width + bumpX);
  let offsetY = (height - displayImg.height + bumpY);

  // Turn moto image into points
  for (let i = 0; i < 1000; i++) {
    let x = random(displayImg.width);
    let y = random(displayImg.height); 
    let col = displayImg.get(x, y);
...
function mousePressed(){
  ...
  //increase cyclespeed if horse is clicked
  if (isMouseOverHorse()) {
    increaseSpeed();
  }
}
...
function isMouseOverHorse() {
  ...
  let offsetX = (width - currentHorseImage.width+ bumpX);
  let offsetY = (height - currentHorseImage.height+ bumpY);
  ...
  if (imgX >= 0 && imgX < currentHorseImage.width && 
    ...
    // Check if it's red outline OR black interior
...
function increaseSpeed() {
  if (cycleSpeed === 0) {  // Increase speed with each click (starting with first click)
    cycleSpeed = 2000; // 2 second between changes
    autoCycle = true;
  } else if (cycleSpeed > 0.1) {
    cycleSpeed = cycleSpeed * 0.7; // 30% faster each click
  } else {
...
//reset poster at the end of sequence
function reset() {
  cycleSpeed = 0;
  autoCycle = false;
  currentHorse = 0;
  ...
  loadHorsePoints(currentHorse);
  points = horsePoints[currentHorse];
}

function checkReset() {
  if (cycleSpeed > 0 && cycleSpeed < 1 && !countdownActive) {
    // Start countdown when speed is maxed
